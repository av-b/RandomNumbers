// GenLognormal.cpp генерация случайных чисел с логнормальным распределением
//
#include "pch.h"
#include <iostream>
#include <cmath>
...
int main()
{
	...
	double mo = 0, sig = 0.5; // задание параметров генератора случайных чисел
	int bins = 100; // количество карманов гистограммы
	int i, n;
	double g[s]; // массив для записи случайных чисел
	lognormal_distribution<> distr(mo, sig); // инициализация генератора сл. чисел
	...
	double M = 0, D = 0;
	double ming = 0, maxg = 2 * sig + mo; // пределы изменения g
	for (i = 0; i < s; i++) {
